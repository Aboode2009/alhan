...
public class AlhanMediaService extends MediaSessionService {
    ...
    @Override public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null && ACTION_PLAY.equals(intent.getAction())) {
            ...
            if (uri != null && !uri.isEmpty()) playUri(Uri.parse(uri), intent.getStringExtra(EXTRA_TITLE), intent.getStringExtra(EXTRA_ARTIST), intent.getStringExtra(EXTRA_ARTWORK));
        }
        return START_STICKY;
    }

    private void playUri(Uri uri, String title, String artist, String artwork) {
        MediaMetadata.Builder metadata = new MediaMetadata.Builder().setTitle(title != null ? title : "Alhan").setArtist(artist != null ? artist : "").setAlbumTitle("ألحان");
    ...
    public static synchronized long getDuration() { if (instance == null || instance.player == null) return 0; long d = instance.player.getDuration(); return d == C.TIME_UNSET ? 0 : Math.max(0, d); }
